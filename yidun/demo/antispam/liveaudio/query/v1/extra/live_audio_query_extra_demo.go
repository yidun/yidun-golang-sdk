...
func main() {
	...
	request := request2.NewLiveAudioQueryExtraV1Req(YourBusinessId)
	// 设置查询开始时间和结束时间
	request.SetStartTime(1701407273000)
	request.SetEndTime(1701407393000)
	...
	response, err := client.QueryExtra(request)
	if err != nil {
		// 处理错误并打印日志
		log.Fatal(err)
	}
