...
func main() {
	...
	request := request2.NewLiveAudioQuerySegmentV1Req(YourBusinessId)
	// 设置查询开始时间和结束时间
	request.SetStartTime(1701262095000)
	request.SetEndTime(1701262215000)
	...
	response, err := client.QuerySegment(request)
	if err != nil {
		// 处理错误并打印日志
		log.Fatal(err)
	}
