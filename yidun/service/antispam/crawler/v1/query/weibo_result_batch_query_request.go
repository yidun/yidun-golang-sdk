...
// WeiboBatchQueryV1Request 网站任务检测批量查询接口v1.0
type WeiboBatchQueryV1Request struct {
...
func NewWeiboBatchQueryV1Request() *WeiboBatchQueryV1Request {
	var request = &WeiboBatchQueryV1Request{
		BizPostFormRequest: types.NewBizPostFormRequestWithoutBizId(),
	}
	request.SetProductCode("crawler")
	request.SetUriPattern("/v1/crawler/weibo-job/batchQuery")
	request.SetMethod(http.HttpMethodPost)
	request.SetVersion("v1.0")
	return request
}

func (j *WeiboBatchQueryV1Request) SetJobIds(jobIds []int64) {
	j.JobIds = &jobIds
}

// 获取参与签名的参数
func (c *WeiboBatchQueryV1Request) GetBusinessCustomSignParams() map[string]string {
...
// 参数校验
func (c *WeiboBatchQueryV1Request) ValidateParam() error {
