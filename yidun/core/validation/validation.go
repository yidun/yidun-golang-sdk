...
const (
	...
	ErrParamBetweenRangeCode = "ParamBetweenRangeError"
	...
	//ParamInvalidErrCode is the error code for a field with invalid
	ParamInvalidErrCode = "ParamInvalidError"
)
...
// OrigErr returns the invalid parameters as a yidunerr.BatchedErrors value
func (e ErrInvalidParams) OrigErr() error {
	return yidunerr.NewBatchError(
...
// OrigErr returns nil, Implemented for yidunerr.Error interface.
...
// An ErrParamInvalid represents an invalid parameter error.
type ErrParamInvalid struct {
	errInvalidParam
}

// NewErrParamInvalid creates a new invalid parameter error.
func NewErrParamInvalid(field string) *ErrParamInvalid {
	return &ErrParamInvalid{
		errInvalidParam{
			code:  ParamInvalidErrCode,
			field: field,
			msg:   fmt.Sprintf("invalid field"),
		},
	}
}
...
type ErrParamBetweenRange struct {
	errInvalidParam
	min float64
	max float64
}

func NewErrParamBetweenRange(field string, min float64, max float64) *ErrParamBetweenRange {
	return &ErrParamBetweenRange{
		errInvalidParam: errInvalidParam{
			code:  ErrParamBetweenRangeCode,
			field: field,
			msg:   fmt.Sprintf("field value should between %v and %v", min, max),
		},
		min: min,
	}
